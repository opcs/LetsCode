...
package com.op.algo.graph;

import java.util.*;

/**
 * This class models a simple, undirected graph using an incidence list representation. Vertices are identified
 * uniquely by their labels, and only unique vertices are allowed.
 * At most one Edge per vertex pair is allowed in this Graph. Note that the Graph is designed to manage the Edges. You
 * should not attempt to manually add Edges yourself.
 */
public class Graph {

    private HashMap<String, Vertex> vertices; // v.label->vertex
    private HashMap<Integer, Edge> edges; // e.hashcode ->e
